class StatisticController < ApplicationController
  ...
  def top_searches
    Rails.cache.fetch('top_searches', :expires_in => 2.hours) do
      result = ActiveRecord::Base.connection.execute("select query, count(*) as c from search_histories where query is NOT NULL AND count > 0 AND created_at > '#{TIME_LIMIT}' group by query order by c desc limit 15;")
      top = Array.new
